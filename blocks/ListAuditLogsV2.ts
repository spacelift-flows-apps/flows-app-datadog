...
export const ListAuditLogsV2: AppBlock = {
  name: "Get a list of Audit Logs events",
  description: `List endpoint returns events that match a Audit Logs search query. [Results are paginated][1].  Use this endpoint to see your latest Audit Logs events.  [1]: https://docs.datadoghq.com/logs/guide/collect-multiple-logs-with-pagination`,
  category: "AuditLogs",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const urlBase = `${baseUrl}/api/v2/audit/events`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to get a list of audit logs events: ${response.status} ${response.statusText} - ${errorText}`);
        }
  ...
  outputs: {
    default: {
      name: "AuditLogs List",
      description: `List of auditlogs objects`,
      default: true,
      type: {
        type: "object",
        properties: {

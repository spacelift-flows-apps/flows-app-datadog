...
export const ListSpansGetV2: AppBlock = {
  name: "Get a list of spans",
  description: `List endpoint returns spans that match a span search query. [Results are paginated][1].  Use this endpoint to see your latest spans. This endpoint is rate limited to \`300\` requests per hour.  [1]: /logs/guide/collect-multiple-logs-with-pagination?tab=v2api`,
  category: "SpansGet",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const urlBase = `${baseUrl}/api/v2/spans/events`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to get a list of spans: ${response.status} ${response.statusText} - ${errorText}`);
        }
  ...
  outputs: {
    default: {
      name: "SpansGet List",
      description: `List of spansget objects`,
      default: true,
      type: {
        type: "object",
        properties: {

...
export const ListCIAppPipelineEventsV2: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const urlBase = `${baseUrl}/api/v2/ci/pipelines/events`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to get a list of pipelines events: ${response.status} ${response.statusText} - ${errorText}`);
        }
  ...
  outputs: {
    default: {
      name: "CIAppPipelineEvents List",
      description: `List of ciapppipelineevents objects`,
      default: true,
      type: {
        type: "object",
        properties: {

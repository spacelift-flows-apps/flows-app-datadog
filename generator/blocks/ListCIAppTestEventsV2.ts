...
export const ListCIAppTestEventsV2: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const urlBase = `${baseUrl}/api/v2/ci/tests/events`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to get a list of tests events: ${response.status} ${response.statusText} - ${errorText}`);
        }
  ...
  outputs: {
    default: {
      name: "CIAppTestEvents List",
      description: `List of ciapptestevents objects`,
      default: true,
      type: {
        type: "object",
        properties: {

...
export const SearchCIAppTestEventsV2: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const url = `${baseUrl}/api/v2/ci/tests/events/search`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to search tests events: ${response.status} ${response.statusText} - ${errorText}`);
        }
  ...
  outputs: {
    default: {
      name: "Created CIAppTestEvents",
      description: `The created ciapptestevents object`,
      default: true,
      type: {
        type: "object",
        properties: {

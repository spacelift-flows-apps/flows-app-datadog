...
export const SearchMonitorsV1: AppBlock = {
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const urlBase = `${baseUrl}/api/v1/monitor/search`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to monitors search: ${response.status} ${response.statusText} - ${errorText}`);
        }

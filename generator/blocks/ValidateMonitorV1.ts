...
export const ValidateMonitorV1: AppBlock = {
  name: "Validate a monitor",
  description: `Validate the monitor provided in the request.  **Note**: Log monitors require an unscoped App Key.`,
  category: "Monitor",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        ...
        const url = `${baseUrl}/api/v1/monitor/validate`;
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to validate a monitor: ${response.status} ${response.statusText} - ${errorText}`);
        }
